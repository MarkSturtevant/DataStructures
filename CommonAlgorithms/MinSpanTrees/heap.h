...
template <class Container, class Compare=std::less<typename Container::value_type>>
void heap_insert(Container* list, const typename Container::value_type& item, Compare lessThan =std::less<typename Container::value_type>{}) {
    list->push_back(item);
    size_t cIndex = list->size() - 1;
    while (cIndex > 1) {
        size_t nIndex = cIndex / 2;
        if (lessThan(item, (*list)[nIndex])) {
            (*list)[cIndex] = (*list)[nIndex];
            cIndex /= 2;
        }
        else break;
    }
...
const typename Container::value_type& heap_get_min(const Container& list) {
    if (list.size() <= 1)
